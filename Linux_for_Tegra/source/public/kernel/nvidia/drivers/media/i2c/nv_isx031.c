...
int isx031_write_table(struct tegracam_device *tc_dev, uint8_t *pdata, int32_t setting_size)
{
	int err=0;
	...
	for (i = 0; i < setting_size;) 
	{
		...
		if (len == 5) 
		{
			...
			err = isx031_write_reg(s_data, reg_addr, reg_val);
			if (err) {
				dev_err(dev,"%s: error\n", __func__);
				//return err;
			}	
			
			err = isx031_read_reg(s_data, reg_addr, &reg_val_read);
			if (err) {
				dev_err(dev,"%s: error\n", __func__);
				//return err;
			}
...
static int isx031_gmsl_serdes_setup(struct isx031 *priv)
{
...
static void isx031_gmsl_serdes_reset(struct isx031 *priv)
{
	mutex_lock(&serdes_lock__);
...
static int isx031_power_get(struct tegracam_device *tc_dev)
{
	...
	dev_info(tc_dev->dev, "isx031_power_get.\n");
	pw->state = SWITCH_OFF;
	return 0;
}
...
static int isx031_set_exposure(struct tegracam_device *tc_dev, s64 val)
{
	...
	dev_info(tc_dev->dev, "isx031_set_exposure.\n");
	return 0;
...
static struct camera_common_pdata *isx031_parse_dt(struct tegracam_device *tc_dev)
{
	struct device *dev = tc_dev->dev;
...
int sensor_init(struct isx031 *priv);
#endif
...
static int isx031_set_mode(struct tegracam_device *tc_dev)
{
	...
	dev_info(dev, "isx031_set_mode111.\n");
...
int sensor_stop(struct tegracam_device *tc_dev) 
{
  ...
  if (ret < 0) 
  {
    dev_err(dev, "isx031 stream_off fail\n");
    return -1;
  }
...
static int isx031_start_streaming(struct tegracam_device *tc_dev)
{
	...
	dev_info(dev, "isx031_start_streaming.\n");
...
static int isx031_stop_streaming(struct tegracam_device *tc_dev)
{
	int err=0;
	struct device *dev = tc_dev->dev;
	dev_info(dev, "isx031_stop_streaming.\n");
...
static int isx031_board_setup(struct isx031 *priv)
{
...
int sensor_init(struct isx031 *priv) 
{
  ...
  while(try_count)
  {
    pdata = isx031_init_setting;
    setting_size = sizeof(isx031_init_setting) / sizeof(uint8_t);
    ...
    ret = isx031_write_table(tc_dev, pdata, setting_size);
    if (ret < 0) 
    {
			...
			const struct i2c_device_id *id)
{
	...
	printk("---isx031_probe 111---\r\n");

	if (!IS_ENABLED(CONFIG_OF) || !node)
		return -EINVAL;

	priv = devm_kzalloc(dev, sizeof(struct isx031), GFP_KERNEL);
	if (!priv) {
	...
	printk("---isx031_probe 444---\r\n");

	err = isx031_gmsl_serdes_setup(priv);
	if (err) {
		dev_err(&client->dev,
...
static int __init isx031_init(void)
{
	mutex_init(&serdes_lock__);
	
	printk("---isx031_init---\r\n");

	return i2c_add_driver(&isx031_i2c_driver);
}

static void __exit isx031_exit(void)
{
	mutex_destroy(&serdes_lock__);
	
	printk("---isx031_exit---\r\n");

	i2c_del_driver(&isx031_i2c_driver);
}

module_init(isx031_init);
module_exit(isx031_exit);

MODULE_DESCRIPTION("Media Controller driver for Sensing isx031");
